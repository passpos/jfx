 ...
 */
public class MediaPlayerDemoApp extends ContentBox {

    public static final boolean SHOWING = false;
    public static final String TITLE = "Media - Demo MediaPlayer";
    private Media media;
    ...
    public void baseDemo() {
        ...
        // 频谱更新间隔（秒）
        mp.setAudioSpectrumInterval(0.2);
        // 以分贝为单位的灵敏度阈值；必须是非正的
        mp.setAudioSpectrumThreshold(-100);
        // 音频频谱中的频带数
        mp.setAudioSpectrumNumBands(20);
    }
    ...
    public void eventDemo() {
        ...
        mp.setAudioSpectrumListener(new AudioSpectrumListener() {
            @Override
            public void spectrumDataUpdate(
                    double timestamp,
                    double duration,
                    float[] magnitudes,
                    float[] phases) {
                ol("magnitudes - " + magnitudes.length);
                ol("phases - " + phases.length);
            }
        });
    }
